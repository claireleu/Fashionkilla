...
def recommend_best_items(last_generated_prompt: list, closet: dict, get_event_description_from_image):
    """
    """
    image_embeddings = []
    ...
    for category, items_list in closet.items():
        ...
        for item in items_list:
            item_embedding = text_model.encode(item.get("keywords", ""), convert_to_tensor=True)
            max_item_score = max(util.cos_sim(item_embedding, img_emb[0]).item() for img_emb in image_embeddings)
